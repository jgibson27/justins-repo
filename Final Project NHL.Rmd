...
# Creating test and training dat
...
# Boost Prediction
pred.boost = predict(boost.NHL.t, data = train, n.trees = 1000, interaction.depth = 4)

# MSE 
boost.mse = mean((pred.boost - train$Salary)^2)
...
reg.func = function(df1, df2) {
  # Boost reg
  boost.NHL = gbm(Salary ~ Age + Contract.Length + t_on_ice + assists + goals + pim + shots + games + hits + pp_goals + pp_points + pp_t_on_ice + even_t_on_ice + pen_min + face_off_perc + shot_perc + game_win_goals + over_t_goals + short_hand_goals + short_hand_points + short_hand_t_on_ice + blocked + plus_minus + points + shifts, data=df1, distribution = "gaussian", n.trees = 1000, interaction.depth = 4)
  
  # Boost Prediction
pred.boost = predict(boost.NHL, data = df1, n.trees = 1000, interaction.depth = 4)

# MSE 
boost.mse = mean((pred.boost - df1$Salary)^2)

boost.mse

  # Test MSE from ridge regression
# Creating matrices
x = model.matrix(Salary ~ ., data=df1)
z = model.matrix(Salary ~ ., data=df2)
y = df1$Salary

x = model.matrix(Salary ~ ., data=train)
z = model.matrix(Salary ~ ., data=test)
y = train$Salary
...
mse_vec = c(boost_mse, ridge_mse,lasso_mse)
name_vec = c("Boost MSE","Ridge MSE","Lasso MSE")
ggplot(data=NULL, aes(x=name_vec,y=mse_vec)) + geom_col(width=0.5)
}
